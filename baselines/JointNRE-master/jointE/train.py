...
def main(_):
	...
	def train_kg(coord):
		...
		while not coord.should_stop():
			...
			print "KG batch %d time %s | loss : %f" % (times_kg, time_str, res)
	...
	def train_nn(coord):
		...
		for one_epoch in range(FLAGS.max_epoch):
			...
			if (one_epoch + 1) % save_epoch == 0:
				...
				print 'saving model...'
				path = saver.save(sess,FLAGS.model_dir+sys.argv[1]+FLAGS.model+str(FLAGS.katt_flag), global_step=current_step)
	...
	print (  FLAGS.model+str(FLAGS.katt_flag)+"-"+str(3664*10))
	print (FLAGS.model_dir + FLAGS.model+str(FLAGS.katt_flag)+"-"+str(3664*10))
	print ("================================================")
	#saver.restore(sess, FLAGS.model_dir + FLAGS.model+str(FLAGS.katt_flag)+"-"+str(3664*10))
