...
import numpy as np
from rl_code.data.feed_data import RelationEntityBatcher
from rl_code.data.grapher import RelationEntityGrapher
import logging
...
class Episode(object):
    ...
    def __init__(self, graph, data, params):
        ...
        :param data:
                ...
                r: shape = (batch_size, ).
                e2: shape = (batch_size, ).
                all_e2s: len = batch_size. It stores all corresponding e2s according to (e1, r).
        :param params:
        :return:
                self.start_entities/current_entities/end_entities/query_relation: shape = (batch_size*num_rollouts, )
                self.state = {'next_relations'/'next_entities':(batch_size*num_rollouts, max_num_actions),
                              'current_entities': (batch_size*num_rollouts, )}
        """
        self.grapher = graph
        ...
        self.no_examples = start_entities.shape[0]      # = batch_size
        self.positive_reward = positive_reward          # a parameter, normally =1
        self.negative_reward = negative_reward          # a parameter, normally =0
...
class env(object):
    def __init__(self, params, mode='train'):
        ...
        self.grapher = RelationEntityGrapher(triple_store=params['data_input_dir'] + '/' + 'train.txt',   # 'graph.txt'
