...
class json_file_data_loader(file_data_loader):
    ...
    def _load_preprocessed_file(self):
        """
        Loading the already preprocessed data file.
        :return:
        """
        ...
        self.entpair_dict = {}
        for pairs in self.entpair2scope:
            start, end = pairs.split("#")
            if start not in self.entpair_dict:
                self.entpair_dict[start] = {}
            self.entpair_dict[start][end] = self.entpair2scope[pairs]

        self.scope2entpair = {tuple(self.entpair2scope[k]): k for k in self.entpair2scope}
    ...
    def __init__(self, file_name, word_vec_file_name, rel2id_file_name, mode, shuffle=True, max_length=120, case_sensitive=False, reprocess=False, batch_size=160):
        ...
        if reprocess or not self._load_preprocessed_file(): # Try to load pre-processed files:
            ...
            # Turn Capital characters into lower case
            ...
            self.instance_tot = len(self.ori_data)  # instance means how many sentences/examples in the train/test file
            ...
            for i in range(self.instance_tot):
                ...
                # turn entpair and relfact into scope
                ...
                if cur_relfact != last_relfact:
                    ...
                    last_relfact_pos = i
                # for calculating the position embedding
                p1 = sentence.find(' ' + head + ' ')    # p1 is the starting position for the first character of ety1
                p2 = sentence.find(' ' + tail + ' ')    # p2 is the starting position for the first character of ety2
                if p1 == -1:
                ...
                for j, word in enumerate(words):
                    ...
                    if cur_pos == p1:
                        pos1 = j    # pos1 is the word position for entity1 "word", Notice: it's not character
                        p1 = -1
                    if cur_pos == p2:
                        pos2 = j    # pos2 is the word position for entity2 "word"
                        p2 = -1
                    cur_pos += len(word) + 1
                for j in range(j + 1, max_length):
                    cur_ref_data_word[j] = BLANK    # Add Padding
                self.data_length[i] = len(words)
                if len(words) > max_length:         # Cut longer sentences
                ...
                # Adding mask and position embedding, use entity1 and entity2 split sentence into 1, 2, 3, 0
        ...
        if self.mode == self.MODE_INSTANCE:
            ...
            for key, value in iteritems(self.entpair2scope):
                self.scope_name.append(key)     # ['C0000294#C0010692', 'C0000503#C0000503', 'C0000677#C0000677', 'C0000726#C0000726', 'C0000726#C0003483', 'C0000726#C0016976']
                self.scope.append(value)        # [[0, 1], [1, 2], [2, 3], [3, 26], [26, 61]]
    ...
    def next_batch(self, batch_size):
        ## new rl_code added
        # Something went wrong (loss = inf) with the last padding batch, so I use this to skip the last batch
        # if self.idx >= len(self.order):
        if self.idx + batch_size >= len(self.order):
        ## new rl_code ended
            self.idx = 0
            if self.shuffle:
                random.shuffle(self.order)
    ...
    def batch_gen(self, current_node):
        ...
        if current_node in self.entpair_dict:
            orders = list(self.entpair_dict[current_node].values())
            # TODO: padding
            batch = self._batch_gen(self.batch_size, orders[: self.batch_size])
    ...
    def _batch_gen(self, batch_size, orders):
        ## new rl_code added
        # Something went wrong (loss = inf) with the last padding batch, so I use this to skip the last batch
        # if self.idx >= len(self.order):
        """
        ...
        :param batch_size: batch_size of this batch
        :param orders: list of selected scope, eg. [[23866, 23871], [23871, 23876], [23876, 23881], [23881, 23886]]
        :return: batches, dtype: dict
        """

        batch_data = {}

        if self.mode == self.MODE_ENTPAIR_BAG or self.mode == self.MODE_RELFACT_BAG:
            _word = []
            _pos1 = []
            _pos2 = []
            _mask = []
            ...
            for o in orders:
                ...
                _rel.append(self.data_rel[o[0]])
                _ins_rel.append(self.data_rel[o[0]:o[1]])
            ...
            for i in range(batch_size - len(orders)):
                _word.append(np.zeros((1, self.data_word.shape[-1]), dtype=np.int32))
                _pos1.append(np.zeros((1, self.data_pos1.shape[-1]), dtype=np.int32))
                _pos2.append(np.zeros((1, self.data_pos2.shape[-1]), dtype=np.int32))
                _mask.append(np.zeros((1, self.data_mask.shape[-1]), dtype=np.int32))
    ...
    def batch_gen_train(self, batch_triples):
        ...
        # TODO: padding
        batch = self._batch_gen_train(self.batch_size, orders, rlts)

        return batch

    def _batch_gen_train(self, batch_size, orders, rlts):
        ## new rl_code added
        # Something went wrong (loss = inf) with the last padding batch, so I use this to skip the last batch
        # if self.idx >= len(self.order):
        """
        ...
        for index, o in enumerate(orders):
            ...
            _pos2.append(self.data_pos2[o[0]:o[1]])
            _mask.append(self.data_mask[o[0]:o[1]])
            _rel.append(rlts[index])    # self.data_rel[o[0]]
            _ins_rel.append(np.repeat(rlts[index], bag_size))   # ???? self.data_rel[o[0]:o[1]]
            _length.append(self.data_length[o[0]:o[1]])
            _scope.append([cur_pos, cur_pos + bag_size])
            cur_pos = cur_pos + bag_size
            if self.mode == self.MODE_ENTPAIR_BAG:
                _one_multi_rel = np.zeros((self.rel_tot), dtype=np.int32)
                for j in range(o[0], o[1]):
                    _one_multi_rel[self.data_rel[j]] = 1    # what is this for?
                _multi_rel.append(_one_multi_rel)
                _entpair.append(self.scope2entpair[tuple(o)])
        for i in range(batch_size - len(orders)):
            # for padding
